...
- **同进程运行**（参考官方 Android 客户端）：`EasyTierVpnService`（`android:process=":vpn"`）
  在独立进程里依次执行 `System.loadLibrary`、`runNetworkInstance(toml)`、
  `Builder().establish()` 建立 TUN、`setTunFd(instance, fd)` 把 fd 交给核心。
- **VPN 声明**：本地静态 IP（`ipv4 = "10.144.144.100/24"`）或 DHCP 占位地址
  `10.144.144.100` 用于先建 TUN；TUN 参数（路由/DNS）通过 service intent 传入。
  核心自带的内置 IP 段为 `10.144.144.0/24`。
...
| 文件 | 修改 | 原因 |
|------|------|------|
| `easytier/Cargo.toml` | `service-manager` 改为 `optional = true` 并加入 `management` feature | 该依赖走 git + 固定分支，移动端非 management 构建不需要它，避免 CI 拉取 |
